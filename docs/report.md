...
**Effetto collaterale dello staleness sul retry.** Il meccanismo di retry non distingue tra peer irraggiungibile (`RpcError`) e peer che ha rifiutato il messaggio per staleness (`Ack.accepted = False`): entrambi producono `send_model() = False` e finiscono in `failed_targets`. Di conseguenza, un rifiuto per staleness trigghera anch'esso il refresh della cache e il tentativo con un peer sostituto. Questo non è il comportamento inteso — il problema non è la raggiungibilità del peer ma il fatto che il nostro round è troppo indietro rispetto al suo — ma l'effetto è comunque ragionevole: il peer sostituto estratto dalla lista fresca potrebbe essere a un round più vicino al nostro e accettare il messaggio. Il caso peggiore, in cui tutti i peer disponibili sono avanti di `max_staleness` round, produce semplicemente push falliti senza conseguenze sul training.
